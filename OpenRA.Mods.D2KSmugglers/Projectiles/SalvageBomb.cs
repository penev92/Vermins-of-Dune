...
namespace OpenRA.Mods.D2KSmugglers.Projectiles
{
	...
	public class SalvageBomb : IProjectile, ISync
	{
		...
		public SalvageBomb(SalvageBombInfo info, ProjectileArgs args)
		{
			...
			rangeLimit = new WDist(Common.Util.ApplyPercentageModifiers(limit.Length, args.RangeModifiers));
		...
		WVec HomingTick(World world, WVec tarDistVec, int relTarHorDist)
		{
			...
			hFacing = Common.Util.TickFacing(hFacing, desiredHFacing, info.HorizontalRateOfTurn.Facing);
			vFacing = Common.Util.TickFacing(vFacing, desiredVFacing, info.VerticalRateOfTurn.Facing);
